...
export default function App() { // Komponen App
  const [animes, setAnimes] = useState(animesData); // State mengelola data anime 
  const [selectedAnime, setSelectedAnime] = useState(animes[0]); // Mengelola data yang tampil disebelah kanan 

  function handleSelectedAnime(id) { // Memilih yang akan tampil disebelah kanan
...
function Search({ children }) {
  const [query, setQuery] = useState(''); // Mengelola query 
  return (
...
function Box({ children }) { // Menerima Props
  const [isOpen, setIsOpen] = useState(true); // Check apakah kotaknya lagi dibuka atau tidak
